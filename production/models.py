...
class TextileStock(models.Model):
    """مخزون الأقمشة"""
    FABRIC_TYPES = [
        ('cotton', 'قطن'),
        ('polyester', 'بوليستر'),
    ...
    fabric_type = models.CharField(max_length=20, choices=FABRIC_TYPES, verbose_name="نوع القماش")
    color = models.CharField(max_length=50, verbose_name="اللون")
    width = models.DecimalField(max_digits=5, decimal_places=2, verbose_name="العرض (سم)")
    cost_per_meter = models.DecimalField(max_digits=8, decimal_places=2, verbose_name="تكلفة المتر")
    supplier = models.ForeignKey('suppliers.Supplier', on_delete=models.SET_NULL, null=True, blank=True, verbose_name="المورد")
    ...
    warehouse_product = models.OneToOneField(
        ...
        null=True,
        blank=True,
        related_name='textile_equivalent',
        verbose_name="منتج المخزن المرتبط"
    )
    ...
    def __str__(self):
        return f"{self.name} - {self.color} ({self.length_available}م)"
    
    @property
    def stock_item(self):
        if self.warehouse_product and self.warehouse:
            try:
    ...
    def length_available(self):
        ...
        return item.quantity if item else Decimal('0.00')

    @property
    def available_quantity(self):
        item = self.stock_item
        return item.available_quantity if item else Decimal('0.00')

class ProductCategory(models.Model):
    """Product Categories with hierarchy"""
    name = models.CharField(max_length=100, verbose_name="اسم الفئة")
    parent = models.ForeignKey('self', on_delete=models.CASCADE, null=True, blank=True, related_name='subcategories', verbose_name="الفئة الأساسية")
    is_active = models.BooleanField(default=True, verbose_name="نشط")
...
class FinishedProduct(models.Model):
    ...
    category = models.ForeignKey(ProductCategory, on_delete=models.SET_NULL, null=True, blank=True, verbose_name="الفئة")
    size_group = models.ForeignKey(SizeGroup, on_delete=models.SET_NULL, null=True, blank=True, verbose_name="مجموعة المقاسات")
    fabric_per_meter = models.DecimalField(max_digits=6, decimal_places=2, default=0, verbose_name="متطلبات القماش لكل متر")
    description = models.TextField(blank=True, verbose_name="الوصف")
    base_cost = models.DecimalField(max_digits=10, decimal_places=2, default=0, verbose_name="التكلفة الأساسية")
    selling_price = models.DecimalField(max_digits=10, decimal_places=2, default=0, verbose_name="سعر البيع")
    fabric_quantity_per_piece = models.DecimalField(max_digits=6, decimal_places=2, verbose_name="كمية القماش المطلوبة لإنتاج قطعة واحدة")
    warehouse_product = models.OneToOneField(
        WarehouseProduct, 
        on_delete=models.SET_NULL, 
        null=True, 
        blank=True, 
        related_name='finished_product_origin',
        verbose_name="منتج المخزن"
    )
    ...
    def calculate_fabric_requirement(self, size=None):
        """Calculate fabric requirement based on size and BOM"""
        base_requirement = self.fabric_quantity_per_piece
        if size and self.size_group:
            # Size multiplier logic
            size_index = self.size_group.sizes.index(size) if size in self.size_group.sizes else 0
            multiplier = 1 + (size_index * 0.1)  # 10% increase per size step
...
class BillOfMaterials(models.Model):
    ...
    A BOM lists all the raw materials required to produce one unit of the product.
    This model supports versioning to track changes over time.
    """
    ...
    def __str__(self):
        """
        String representation of the BOM object.
        """
        size_group_name = self.size_group.name if self.size_group else "Default"
        return f"BOM for {self.product.name} ({size_group_name}) - v{self.version}"

    @property
    def total_cost(self):
        """
        Calculates the total cost of all materials in this BOM.
        It aggregates the cost of each BOMItem (quantity * material cost price).
        """
        # The related name for BOMItem is 'items'
...
class BOMItem(models.Model):
    ...
    material = models.ForeignKey(WarehouseProduct, on_delete=models.PROTECT, related_name='bom_items', verbose_name="المادة الخام")
...
class ProductionOrder(models.Model):
    ...
    product = models.ForeignKey(FinishedProduct, on_delete=models.CASCADE, verbose_name="المنتج")
    ...
    textile_stock = models.ForeignKey(TextileStock, on_delete=models.CASCADE, verbose_name="مخزون القماش")
...
class FinishingProcess(models.Model):
    ...
    destination_warehouse = models.ForeignKey('warehouses.Warehouse', on_delete=models.SET_NULL, null=True, blank=True, verbose_name="المخزن النهائي")
...
class FinalProduct(models.Model):
    """المنتج النهائي المخزن"""
    finishing_process = models.ForeignKey(FinishingProcess, on_delete=models.CASCADE, related_name='final_products', verbose_name="عملية التشطيب")
    warehouse = models.ForeignKey('warehouses.Warehouse', on_delete=models.CASCADE, verbose_name="المخزن")
    ...
    # التواريخ
    stored_date = models.DateTimeField(default=timezone.now, verbose_name="تاريخ التخزين")
    expiry_date = models.DateField(null=True, blank=True, verbose_name="تاريخ الانتهاء")
    
    stored_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, verbose_name="خزن بواسطة")
    ...
    def calculate_total_cost(self):
        ...
        # تكلفة القماش
        fabric_cost = (production_order.fabric_meters_allocated * production_order.textile_stock.cost_per_meter)
        
        # تكلفة التجميع
        assembly_cost = self.finishing_process.dyeing_process.assembly_process.assembly_cost
        
        # تكلفة الصباغة
        dyeing_cost = self.finishing_process.dyeing_process.total_dyeing_cost
        
        # تكلفة التشطيب
        finishing_cost = self.finishing_process.total_finishing_cost
        
        # إجمالي التكلفة
        self.total_production_cost = fabric_cost + assembly_cost + dyeing_cost + finishing_cost
        
        # تكلفة القطعة
        if self.quantity > 0:
            self.cost_per_piece = self.total_production_cost / self.quantity
